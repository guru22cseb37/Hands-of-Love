...
import { Send, User, MessageSquare, Loader2, Trash2, Mic, X, PhoneCall } from 'lucide-react';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-green-600 flex items-center gap-1">
                <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" /> {activeVoice ? 'Live Voice Session' : 'Real-time Live'}
              </p>
            </div>
          </div>
            ...
            {username && (
              <button 
                ...
                className={`flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-full transition-all ${
                  ...
                  : 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-md hover:scale-105'
                }`}
              >
                {activeVoice ? <X className="w-3 h-3" /> : <Mic className="w-3 h-3" />}
                {activeVoice ? 'Leave Voice' : '🎙️ Start Voice Hangout'}
              </button>
            )}
            {username && (
              <div className="hidden md:flex items-center gap-2 text-xs bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-full dark:text-white">
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          {activeVoice ? (
            <div className="flex-1 w-full h-full bg-gray-900 relative">
              <iframe 
                src={`https://meet.jit.si/AnbinKaram_CommunityRoom_Private#config.prejoinPageEnabled=false&userInfo.displayName="${username}"`} 
